...
if not (do_baseline or do_individual_vars or do_categories):
  print "Done" 
else:
  ...
  if do_individual_vars:
	...
	os.system("exit(1)")
      ...
      if not args.dry_run:
	# Then, train the bdt
	print "python bdt_ducks.py '%s' '%s' '%s' '%s' '%s'" % (args.channel, args.selection, args.year, "add_'%s'" % info["name"], args.n_trainings)
	os.system("python bdt_ducks.py '%s' '%s' '%s' '%s' '%s'" % (args.channel, args.selection, args.year, "add_'%s'" % info["name"], args.n_trainings))
	# Then, calculate <Max Z_A>_N and estimated uncertainty 
	info["results"] = calc_za_and_unc("Optimization/ZA_curves/MVAOptimizationBaby_*_%s_add_%s_*_bdt.npz" % (args.channel, info["name"])) 
...
if do_table:
  ...
  for var, info in vars.iteritems():
      ...
      print "%s & %.3f $\\pm$ %.3f & %.3f $\\pm$ %.3f \\\\ \\hline" % (info["latex_name"], (feature_score["mean_mc"] - mean_mc) / (mean_mc * 0.01), (feature_score["mean_unc_mc"]) / (feature_score["mean_mc"] * 0.01), (feature_score["mean_data"] - mean_data) / (mean_data * 0.01), (feature_score["mean_unc_data"]) / (feature_score["mean_data"] * 0.01)) 
