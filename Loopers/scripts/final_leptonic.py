...
if do_looping:
    ...
    for plot_type in plot_types:
        parallel_utils.run('python plot_wrapper.py --input_file "../ttHLeptonic_RunII_MVA_Presel_%s_histogramsRunII.root" --backgrounds "DiPhoton|GammaJets|TTGG|TTGJets|TTJets|VG|DY" --signals "ttH" --plot_type "%s" --plot_labels "ttH Leptonic|Loose MVA Presel."' % (args.tag, plot_type))
...
if do_dnn:
    ...
    #parallel_utils.run('python train_dnn.py --input "ttHLeptonic_%s_RunII_MVA_Presel_dnn_features_ttGG.hdf5" --tag "ttHLeptonic_ttH_vs_ttGG_%s" --channel "Leptonic" --preprocess_scheme "preprocess_scheme_Leptonic_ttGG.json"' % (args.tag, args.tag), False)
