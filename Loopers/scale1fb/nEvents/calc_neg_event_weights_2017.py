...
with open("samples_dirs.json") as json_file:
  sample_dirs = json.load(json_file)
  dirs = glob.glob("/eos/cms//store/group/phys_higgs/cmshgg/sethzenz/flashgg/RunIIFall17-3_1_0/3_1_0/*")
  for dir in dirs:
    if "Single" in dir or "Double" in dir:
      continue
    print dir
    microAOD_list = []
    ...
    sample_map[sample] = microAOD_list
      #for samp, directory in sample_dirs.iteritems():
      #  if directory in subdir and dir.split("/")[-1] == samp:
      #    for root, dirnames, filenames in os.walk(dir):
      #      for filename in fnmatch.filter(filenames, '*.root'):
      #        microAOD_list.append(os.path.join(root, filename))
      #        sample = dir.split("/")[-1]
...
with open("calc_neg_event_weights_2017.cpp", "w") as fout:
  ...
  for key, entries in sample_map.iteritems():
      #if "WJets" not in key and "TTJets" not in key: 
      #  continue
    ...
    fout.write('  cout << " \'%s\'" << " : [" << frac_neg_weights_%s << " , " << pos_weights_%s + neg_weights_%s << " ] " << endl;\n' % (key, key.replace("-", "_"), key.replace("-", "_"), key.replace("-", "_")))
    fout.write('  delete ch_%s ; \n' % key.replace("-", "_"))
  fout.write('  cout << " } " << endl;\n')
  fout.write("}\n")
